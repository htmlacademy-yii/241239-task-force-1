        ...
        <div class="content-view__feedback-card user__search-wrapper">
            <div class="feedback-card__top">
                <div class="user__search-icon">
                    <a href="<?= Url::to(['/users/view/' . $user->id]); ?>"><img src="img/man-glasses.jpg" width="65" height="65"></a>
                    <span><?= $user->countTasks; ?> заданий</span>
                    <span><?= $user->userReviews; ?> отзывов</span>
                </div>
                <div class="feedback-card__top--name user__search-card">
                    <p class="link-name"><a href="<?= Url::to(['/users/view/' . $user->id]); ?>" class="link-regular"><?= Html::encode($user->userInfos->name . ' ' . $user->userInfos->surname);  ?></a></p>
                    <?=Yii::$app->customHelper->renderRating($user->userInfos->rating)?>
                    <b><?= Html::encode($user->userInfos->rating); ?></b>
                    <p class="user__search-content">
                        <?= Html::encode($user->userInfos->bio); ?>
                    </p>
                </div>
